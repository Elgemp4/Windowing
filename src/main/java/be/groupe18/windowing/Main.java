...
public class Main extends Application {
  ...
  private MainViewModel assembleDependencies(Stage primaryStage) {
    ...
    SimpleSplitStrategy<Segment> simpleSplitStrategy =
      new LinearSimpleSplitStrategy<>();
    ...
    IFileDialogService fileDialogService = new JavaFXFileDialogService(
      primaryStage
    );
    ...
    IWindowQueryingService windowQueryingService = new QueryWindowService(
      queryStrategy,
      pstRepository
    );
    ...
    return new MainViewModel(
      ...
      segmentRepository,
      sceneQueryService,
      windowQueryingService
    );
  }

  private void initializeView(Stage primaryStage, MainViewModel mainViewModel)
    throws IOException {
    FXMLLoader fxmlLoader = new FXMLLoader(
      getClass().getResource(MAIN_FXML_PATH)
    );

    fxmlLoader.setControllerFactory(type ->
      createController(type, mainViewModel)
    );
  ...
  private Object createController(Class<?> type, MainViewModel mainViewModel) {
    ...
    try {
      ...
      System.err.println(
        "Erreur lors de l'instanciation du contrôleur : " + type.getName()
      );
      e.printStackTrace();
      System.exit(1);
