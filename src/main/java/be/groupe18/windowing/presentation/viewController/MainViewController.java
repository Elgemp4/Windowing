...
public class MainViewController {
    ...
    @FXML private NumericTextField xMinField;
    @FXML private NumericTextField xMaxField;
    @FXML private NumericTextField yMinField;
    @FXML private NumericTextField yMaxField;
    ...
    public void initialize() {
        ...
        mainCanvas.widthProperty().addListener((observable, oldValue, newValue) -> draw());
        mainCanvas.heightProperty().addListener((observable, oldValue, newValue) -> draw());
        ...
        viewModel.segmentsProperty().addListener((observable, oldValue, newValue) -> {
            if(newValue != null) {
                draw();
            }
        });
    ...
    private void drawSegments(List<Segment> segments) {
        ...
        if (Double.isInfinite(renderXMin) || Double.isInfinite(renderXMax) ||
            Double.isInfinite(renderYMin) || Double.isInfinite(renderYMax)) {
        ...
        if (renderXMax - renderXMin == 0) { renderXMin -= 10; renderXMax += 10; }
        if (renderYMax - renderYMin == 0) { renderYMin -= 10; renderYMax += 10; }
        ...
        for (Segment segment : segments) {
            double x1 = (segment.getFirstPoint().getX().getAsDouble() - renderXMin) * scaleX;
            double x2 = (segment.getSecondPoint().getX().getAsDouble() - renderXMin) * scaleX;

            double y1 = height - ((segment.getFirstPoint().getY().getAsDouble() - renderYMin) * scaleY);
            double y2 = height - ((segment.getSecondPoint().getY().getAsDouble() - renderYMin) * scaleY);
