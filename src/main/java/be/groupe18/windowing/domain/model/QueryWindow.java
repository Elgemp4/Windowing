...
public class QueryWindow {
   ...
   */
  public static Tuple<QueryWindow, QueryWindow> buildQueryWindows(
    double minX,
    double maxX,
    double minY,
    double maxY
  ...
  public boolean contains(Segment segment) {
    return (
      isOriginInRange(segment.getOrigin()) &&
      isIntervalInRange(segment.getInterval())
    );
  }
  ...
  private boolean isOriginInRange(CompositeDouble origin) {
    boolean lowerBoundOk =
      (originMin == Double.NEGATIVE_INFINITY) ||
      origin.getAsDouble() >= originMin;
    boolean upperBoundOk =
      (originMax == Double.POSITIVE_INFINITY) ||
      originMax >= origin.getAsDouble();
  ...
  public boolean isIntervalInRange(Interval interval) {
    boolean lowerBoundOk =
      (intervalMin == Double.NEGATIVE_INFINITY) ||
      interval.getIntervalMax().getAsDouble() >= intervalMin;
    boolean upperBoundOk =
      (intervalMax == Double.POSITIVE_INFINITY) ||
      intervalMax >= interval.getIntervalMin().getAsDouble();
    return lowerBoundOk && upperBoundOk;
  }
  ...
  public double getOriginMin() {
    return originMin;
  }

  public double getOriginMax() {
    return originMax;
  }
